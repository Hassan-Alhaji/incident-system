...
const LocationPickerMap: React.FC<LocationPickerMapProps> = ({ onLocationConfirm, initialUrl, onCaptureMap, initialPosition }) => {
 ...
 disabled={isLocating}
 className="absolute top-3 right-3 z-[400] bg-white border border-gray-300 shadow-lg p-2 rounded-lg text-blue-500 hover:bg-slate-200 flex items-center justify-center gap-1.5 transition-all text-base font-medium"
 ...
 {locationError && (
 <div className="flex items-center gap-2 text-red-400 text-base bg-red-500/10 border border-red-500/30 rounded-lg p-2 mb-2">
 ...
 <CheckCircle size={16} />
 <span className="font-bold text-base">{t('map.confirmed')}</span>
 </div>
 ) : (
 <button
 onClick={(e) => { e.preventDefault(); handleConfirm(); }}
 className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-blue-800 hover:from-amber-600 hover:to-orange-700 text-white font-bold py-2 px-5 rounded-lg shadow-lg shadow-amber-500/20 transition-all flex justify-center items-center gap-2 text-base"
 >
 <MapPin size={14} />
 ...
 <div className="text-base text-blue-500 bg-blue-600/10 px-4 py-2 rounded-lg border border-blue-600/30 flex items-center gap-2">
 <MapPin size={14} />
