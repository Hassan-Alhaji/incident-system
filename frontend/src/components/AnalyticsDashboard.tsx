...
const TYPE_COLORS: Record<string, string> = {
 ...
 SPORT: '#8b5cf6',
 MEDICAL: '#ec4899',
 SECURITY_BREACH: '#06b6d4',
 HEALTH: '#8b5cf6',
 FIRE: '#f97316',
 PROPERTY_DAMAGE: '#64748b',
 OTHER: '#94a3b8',
};
const CATEGORY_COLORS_FALLBACK = ['#3b82f6', '#f59e0b', '#10b981', '#8b5cf6', '#ec4899'];
...
const AnalyticsDashboard = () => {
 ...
 return (
 <div className="bg-gray-900/90 backdrop-blur-sm text-gray-800 px-4 py-2 rounded-lg shadow-md text-base border border-gray-700">
 ...
 <p className="text-base font-medium text-gray-500 mt-0.5">Live operational statistics & metrics</p>
 </div>
 </div>
 <div className="hidden sm:flex items-center gap-2 text-base font-bold px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-100">
 ...
 <p className="text-base font-bold text-gray-400 uppercase tracking-widest mb-1">Session Incidents</p>
 ...
 <div className="relative z-10">
 <p className="text-base font-bold text-gray-400 uppercase tracking-widest mb-1">Action Required</p>
 <div className="flex items-baseline gap-2">
 <p className="text-3xl font-black text-gray-900">{openCount + escalatedCount}</p>
 {escalatedCount > 0 && <span className="text-base font-bold text-orange-500 bg-orange-50 px-2 rounded-full border border-orange-100">{escalatedCount} Escalated</span>}
 </div>
 </div>
 ...
 <div className="relative z-10">
 <p className="text-base font-bold text-gray-400 uppercase tracking-widest mb-1">Avg Medical</p>
 ...
 <div className="relative z-10">
 <p className="text-base font-bold text-gray-400 uppercase tracking-widest mb-1">Avg Safety</p>
 ...
 <Cell key={`cell-${index}`} fill={TYPE_COLORS[entry.name] || CATEGORY_COLORS_FALLBACK[index % CATEGORY_COLORS_FALLBACK.length]} />
 ))}
 </Pie>
 ...
 <span className="w-2 h-2 rounded-full" style={{ backgroundColor: TYPE_COLORS[entry.name] || CATEGORY_COLORS_FALLBACK[index % CATEGORY_COLORS_FALLBACK.length] }}></span>
 ...
 <div className="flex-1 flex items-center justify-center text-base font-medium text-gray-400 bg-gray-50/50 rounded-xl shadow-sm border border-dashed border-gray-200">
 ...
 <div className="text-center text-base text-gray-400 font-medium">No priority data logged</div>
 )}
 </div>
 </div>
 ...
 <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col">
 <div className="flex items-center gap-2 mb-6">
 <div className="p-1.5 rounded-lg bg-indigo-50 text-indigo-600"><Target size={16} /></div>
 <h3 className="text-gray-900 font-bold tracking-tight">Top Departments (Incidents)</h3>
 </div>
 <div className="space-y-4 flex-1">
 {data.topDepartments && data.topDepartments.length > 0 ? (
 data.topDepartments.map((dept: any, index: number) => {
 const maxCount = data.topDepartments[0].count;
 const widthPct = Math.max(10, Math.round((dept.count / maxCount) * 100));
 return (
 <div key={index} className="space-y-1.5">
 <div className="flex justify-between text-base font-bold">
 <span className="text-gray-700">{dept.name}</span>
 <span className="text-gray-500">{dept.count} Violations</span>
 </div>
 <div className="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
 <div 
 className="h-2.5 rounded-full relative bg-indigo-500" 
 ...
 <div className="text-center text-base text-gray-400 font-medium">No department data</div>
 )}
 </div>
 </div>

 {/* 6. Service Providers Analytics */}
 <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm col-span-1 lg:col-span-2 flex flex-col">
 <div className="flex items-center justify-between mb-6">
 <div className="flex items-center gap-2">
 <div className="p-1.5 rounded-lg bg-pink-50 text-pink-600"><AlertOctagon size={16} /></div>
 <h3 className="text-gray-900 font-bold tracking-tight">Service Providers (Incidents/Blacklist)</h3>
 </div>
 </div>
 {data.topServiceProviders && data.topServiceProviders.length > 0 ? (
 <div className="overflow-x-auto">
 <table className="w-full text-left">
 <thead>
 <tr className="border-b border-gray-100">
 <th className="pb-3 text-base font-bold text-gray-400 uppercase tracking-wider">Provider Name</th>
 <th className="pb-3 text-base font-bold text-gray-400 uppercase tracking-wider">CR Number</th>
 <th className="pb-3 text-base font-bold text-gray-400 uppercase tracking-wider">Dept</th>
 <th className="pb-3 text-base font-bold text-gray-400 uppercase tracking-wider text-right">Incidents</th>
 <th className="pb-3 text-base font-bold text-gray-400 uppercase tracking-wider text-right">Status</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-50 text-base">
 {data.topServiceProviders.map((sp: any) => (
 <tr key={sp.id}>
 <td className="py-4 font-bold text-gray-900">{sp.name}</td>
 <td className="py-4 text-gray-500 font-mono text-base">{sp.commercialRegistrationNumber}</td>
 <td className="py-4 text-gray-600">{sp.departmentName}</td>
 <td className="py-4 text-right font-bold text-pink-600">{sp.count}</td>
 <td className="py-4 text-right">
 <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${sp.status === 'BLACKLISTED' ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-emerald-100 text-emerald-700 border border-emerald-200'}`}>
 {sp.status}
 </span>
 </td>
 </tr>
 ...
 <div className="flex-1 flex items-center justify-center text-base font-medium text-gray-400 bg-gray-50/50 rounded-xl shadow-sm border border-dashed border-gray-200">
 No Service Provider incidents recorded
 </div>
 )}
 </div>
 </div>
