...
const OfflineSyncManager = () => {
    ...
    if (!isOnline || pendingCount > 0) {
        return (
            <div className={`fixed bottom-4 right-4 z-50 rounded-lg shadow-lg flex items-center gap-3 px-4 py-3 text-sm font-bold text-white transition-all
                ${!isOnline ? 'bg-red-500' : 'bg-yellow-500'}`}
            >
                {!isOnline ? <CloudOff size={20} /> : <Cloud size={20} />}
                <div>
                    {!isOnline ? 'Offline Mode' : 'Syncing Data...'}
                    {pendingCount > 0 && <span className="ml-2 bg-white/20 px-2 py-0.5 rounded-full text-xs">{pendingCount} pending</span>}
                </div>
