...
const MarshalLayout = () => {
 ...
 <button onClick={logout} className="mt-4 text-base text-blue-600 underline">Logout</button>
 </div>
 </div>
 );
 }
 ...
 {/* Mobile Header */}
 <header className="bg-black text-gray-800 p-4 sticky top-0 z-50 shadow-md flex justify-between items-center">
 <div
 ...
 <div>
 <h1 className="font-bold text-base">{user.firstName ? `${user.firstName} ${user.lastName}` : (user.name || 'User')}</h1>
 <p className="text-base text-gray-400">{user.role?.replace(/_/g, ' ') || 'Marshal'}</p>
 </div>
 </div>
 <button onClick={() => { logout(); navigate('/marshal/login'); }} className="text-gray-400 hover:text-gray-800">
 <LogOut size={20} />
 </button>
 </header>
