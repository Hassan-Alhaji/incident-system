...
const STEPS_MEDICAL = [
 { id: 'info', title: 'Type', icon: AlertTriangle }, // Merged Info/Type in Step 0
...
const MarshalTicketForm = () => {
 const navigate = useNavigate();
 const { user } = useAuth();
 const isMedicalRole = user?.isMedical || user?.role?.includes('MEDICAL');
 ...
 eventName: 'Saudi Grand Prix 2026',
 venue: 'Jeddah Corniche Circuit',
 dateTime: new Date().toISOString().slice(0, 16),
 location: '',
 postNumber: '', // Header POST #
 description: '',
 priority: 'MEDIUM',
 ...
 // Header
 headerDriverName: '', // "Driver's Name" in Header
 ...
 motorsportId: '',
 patientCarNo: '',
 patientRole: '', // DRIVER, CO_DRIVER etc
 ...
 // License
 licenseAction: '', // SUSPENDED, NOT_SUSPENDED

 // Injury
 injuryType: '', // PERSONAL_INJURY, NO_PERSONAL_INJURY
 treatmentLocation: '', // COLLISION_SCENE, MEDICAL_CENTRE, OTHER
 treatmentLocationDetail: '',
 arrivalMethod: '', // FOOT, AMBULANCE, NON_MEDICAL_VEHICLE
 incidentDescription: '',
 ...
 initialCondition: '',
 treatmentGiven: '',
 subsequentTreatment: [] as string[], // Multi-select
 subsequentDetail: '',
 ...
 setSubType(sub);
 // Reset steps to 0 relative to new flow? Actually stay at 0 (Type selection is step 1 in General, Step 0 in Medical logic if we merge)
 // Adjusting: If Medical is selected, we are effectively at Step 0 of Medical Flow if we treat Type Selection as Step 0.
 // Let's simpler: If type changes, reset step to 1 (next step).
 };
 ...
 location: formData.location, // General location
 postNumber: formData.postNumber, // Specific Post #
 ...
 {/* But if user picks medical, the steps array changes. */}
 {(activeSteps[currentStep].id === 'type' || (subType === 'MEDICAL' && currentStep === 0)) && (
 <div className="space-y-8 max-w-3xl mx-auto">
 {/* Event Details Input */}
 <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 shadow-sm space-y-4">
 <h3 className="font-bold text-gray-900">Event Details</h3>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="md:col-span-2">
 <label className="block text-base font-bold text-gray-500 uppercase">Event Name</label>
 <input className="w-full border p-2 rounded" value={formData.eventName} onChange={e => updateField('eventName', e.target.value)} />
 </div>
 <div>
 <label className="block text-base font-bold text-gray-500 uppercase">Post Number</label>
 <input className="w-full border p-2 rounded" placeholder="e.g. 14.2" value={formData.postNumber} onChange={e => updateField('postNumber', e.target.value)} />
 </div>
 <div>
 <label className="block text-base font-bold text-gray-500 uppercase">Date</label>
 <input type="datetime-local" className="w-full border p-2 rounded" value={formData.dateTime} onChange={e => updateField('dateTime', e.target.value)} />
 </div>
 </div>
 </div>
 <h2 className="text-xl font-bold text-center">Select Report Type</h2>
 {/* Category Selector */}
 {!eventCategory ? (
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 {/* Types Logic - Keeping Simplified */}
 ...
 {/* Show Event Info Readonly if Type Selected */}
 {selectedType && (
 <div className="mt-8 p-4 bg-gray-50 rounded-lg border border-gray-200">
 <h3 className="font-bold text-base text-gray-700 mb-2">Current Event Confirmation</h3>
 <div className="grid grid-cols-3 gap-4 text-base">
 <div><label className="text-gray-500">Event</label><div>{formData.eventName}</div></div>
 <div><label className="text-gray-500">Venue</label><div>{formData.venue}</div></div>
 <div><label className="text-gray-500">Date</label><div>{formData.dateTime.replace('T', ' ')}</div></div>
 </div>
 </div>
 )}
 </div>
 ...
 {/* --- NON-MEDICAL FLOW [Legacy Steps] --- */}
 {/* General Info Step (Merged logic above handles type) */}
 {/* General Details */}
 ...
 {/* Pit/Grid Specifics */}
 {subType === 'PIT_GRID' && (
 <div className="bg-emerald-50 p-4 rounded-xl shadow-sm border border-emerald-100 grid grid-cols-2 gap-4">
 <div>
 <label className="block text-base font-medium text-emerald-900 mb-1">Car Number</label>
 ...
 <div>
 <label className="block text-base font-medium text-emerald-900 mb-1">Session</label>
 ...
 {/* Witnesses (General) */}
 {/* Note: Witnesses step is separate in General flow, so just basic details here */}
 ...
 {/* General Witnesses Step (Only for Non-Medical, Medical doesn't list it as separate section in spec) */}
 ...
 {/* Section 8 Reference in Spec: Attachments */}
 ...
 <h4 className="font-bold text-gray-800 mb-4 border-b pb-2 flex items-center gap-2">
 <AlertTriangle size={18} className="text-blue-500" /> Incident Report Details
 </h4>
 <div className="grid grid-cols-2 gap-4 text-base">
 ...
 <button onClick={handleNext} disabled={isSubmitting} className="px-8 py-3 rounded-xl shadow-sm bg-emerald-600 text-white font-bold hover:bg-emerald-700 disabled:bg-gray-300 shadow-lg shadow-emerald-200 transition-all flex items-center gap-2">
...
export default MarshalTicketForm;
