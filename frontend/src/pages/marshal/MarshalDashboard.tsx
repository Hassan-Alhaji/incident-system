...
const MarshalDashboard = () => {
 ...
 className="w-full bg-red-600 text-gray-800 p-4 rounded-xl shadow-lg flex items-center justify-center gap-3 text-lg font-bold hover:bg-red-700 transition"
 >
 <PlusCircle size={28} />
 Create New Ticket
 </button>
 ...
 <span className="bg-orange-100 text-orange-800 text-base px-2 py-1 rounded-full">Returned</span>
 </div>
 <p className="text-base text-gray-600 mt-1 line-clamp-1">{ticket.description}</p>
 </div>
 ))}
 </div>
 </div>
 ...
 <div className="text-center py-8 bg-white rounded-xl shadow-sm border border-gray-100 text-gray-400">
 ...
 <span className={`text-base px-2 py-1 rounded-full ${ticket.status === 'OPEN' ? 'bg-emerald-100 text-emerald-800' :
